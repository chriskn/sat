...
def plot_heatmap(data_frame, title, folder, file_name):
    ...
    if number_of_entries > _MAX_HEATMAP_ENTRIES:
        _LOGGER.warning("Number of entries is %d and exceeds limit of %d for heatmaps. Will skip creation of heatmap",
    ...
    # plot
    sns.heatmap(data_frame, square=True, fmt="d", ax=axis,
...
def plot_treemap(data, title, folder, file_name, value_name):
    ...
    norm_values = squarify.normalize_sizes(values, 700., 433.)
    # colors
    ...
    axis = squarify.plot(sizes=norm_values, color=colors,
                         label=labels, ax=axis, alpha=.7)
    axis.set_xticks([])
    axis.set_yticks([])
...
def plot_stacked_barchart(data, ylabel, title, folder, file_name):
    ...
    legend = plt.legend([bottom_bar, top_bar], [data_column_1, data_column_2], loc=1, ncol=2, prop={'size': 16})
