...
_PARSER = argparse.ArgumentParser(formatter_class=RawTextHelpFormatter)


def run_cli(deps_analysers, change_analysers, loc_analysers):
    analyser_types = _PARSER.add_subparsers(
    ...
    _PARSER.add_argument("-v", "--version", action="version", version=_VERSION)


def _add_comp_analysers(analyser_types, locanalyser_names):
...
def _add_deps_analysers(analyser_types, depsanalyser_names):
    depsanalysers = analyser_types.add_parser("deps", help="Dependency analysers")
...
def _add_change_analysers(analyser_types, changeanalyser_names):
    changeanalysers = analyser_types.add_parser("changes", help="Change analysers")
