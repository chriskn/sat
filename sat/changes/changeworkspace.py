...
class ChangeWorkspace(Workspace):
    def __init__(self, directory, ignored_path_segments, since):
        ...
        self._changes_by_file = parser.parse_changes(directory, since)
        self.since = since
    ...
    def create_sourcefile(self, java_file_path, abs_dir, packagename=""):
        w_sourcefile = super().create_sourcefile(
            java_file_path, abs_dir, packagename, parse=False
        )
        changes = (
            self._changes_by_file[java_file_path]
            if java_file_path in self._changes_by_file
            else []
        )
        return SourceFile(
            w_sourcefile.abs_path, w_sourcefile.rel_path, w_sourcefile.name, changes
        )
