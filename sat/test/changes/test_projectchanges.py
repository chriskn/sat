...
class ProjectChangesTest(unittest.TestCase):
    ...
    def test_name_is_packages(self):
        self.assertEquals(ProjectChanges.name(), "projects")
    ...
    def test_write_reults_calls_plot_as_expected(self, dummy_writer, plot):
        ...
        plot.assert_called_once_with(ANY, "Number of changed lines per project since " +
                              self.expected_since, exp_output_folder, "changed_lines_per_project.pdf", "changes:")
    ...
    def test_write_reults_plots_expected_dataframe(self, dummy_writer, plot, change_repo, scanner):
        ...
        self.sut.load_data("","")
        result = self.sut.analyse("")
        exp_treemap_data = result.drop(columns=[ProjectChanges._COLUMNS[0], ProjectChanges._COLUMNS[3], ProjectChanges._COLUMNS[4]])
