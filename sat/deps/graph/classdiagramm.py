...
class ClassDiagramm(Graph):
    ...
    def __init__(self, packages):
        Graph.__init__(self)
        (top_level_elements, self._imports_for_fqn) = _collect_top_level_elements(packages)
    ...
    def _add_interface(self, interface, package_group):
        uml = _uml(interface)
        self.add_node(interface.name, package_group, node_type="UMLClassNode",
...
def _uml(type_):
    attributes = [_declaration_to_string(
        attribute) for attribute in type_.attributes]
...
def _method_to_string(method):
    params = [_declaration_to_string(param).strip(
        " ") for param in method.parameters]
    modifiers = [_visibility(modifier)
...
def _declaration_to_string(attribute):
    modifiers = "".join([_visibility(modifier)
...
def _filter_external_types(class_):
    if not isinstance(class_, Enum):
        types_for_attributes = list(
            filter(
                lambda attr: attr.typename not in _BUILD_IN_TYPES,
                class_.attributes))
