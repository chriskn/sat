...
class BundleParser:
    ...
    def _parse_bundle(self,bundle_path):
        ...
        symbolic_name, version, exportedPackages, importedPackages, requiredBundles = self._parse_manifest(manifest_path)
        number_of_dependencies = len(requiredBundles)+len(importedPackages)
        bundle = Bundle(bundle_path, symbolic_name, version, exportedPackages, importedPackages, requiredBundles, number_of_dependencies)
        return bundle
    ...
    def _parse_manifest(self,manifest_path):
        ...
        for entry in entries: 
            if entry.startswith("Require-Bundle:"):
                required_bundles = [self._trim(bundle) for bundle in self._splitEntries(entry,"Require-Bundle:")]
            elif entry.startswith("Import-Package:"):
                imported_packages = [self._trim(package) for package in self._splitEntries(entry,"Import-Package:")]
            elif entry.startswith("Export-Package:"):
                # removes 'uses' declaration
                if ';' in entry: entry = entry[:entry.index(';')]
                exported_packages = [self._trim(package) for package in self._splitEntries(entry,"Export-Package:")]
                ...
                symbolic_name = self._trim(entry.replace("Bundle-SymbolicName:", ""))
                if ";" in symbolic_name:
                    symbolic_name = symbolic_name[:symbolic_name.index(";")]
        # remove additional information (ie. bundle-version)
        required_bundles[:] = [requiredBundle[:requiredBundle.index(";")]  if ";" in requiredBundle else requiredBundle for requiredBundle in required_bundles]
