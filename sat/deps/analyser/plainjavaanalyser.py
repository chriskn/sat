...
class PlainJavaAnalyser(Analyser):
    ...
    def _write_results_for_projects(self, output_dir):
        ...
        _write_graphml(os.path.join(
            output_dir, "project_dependencies.graphml"), self._project_graph)
        _write_graphml(
            os.path.join(
                output_dir,
                "cyclic_project_dependencies.graphml"),
            self._cycle_project_graph)
        _write_cycles_to_txt(os.path.join(
            output_dir, "project_cycles.txt"), self._project_cycles)
        plot.plot_heatmap(self._project_coupling_data_frame, "Project Coupling",
    ...
    def _write_results_for_packages(self, output_dir):
        ...
        plot.plot_heatmap(self._package_coupling_data_frame, "Package Coupling",
...
def _project_coupling_data_frame(projects):
    ...
    for project in reversed(projects):
        proj_imports = [_to_package_import(
...
def _create_package_coupling_data_frame(packages):
    ...
    for package in reversed(packages):
        package_imps = [_to_package_import(
